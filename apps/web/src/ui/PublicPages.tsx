...
export function Catalog({products}:{products:Product[]}){const [query,setQuery]=useState('');const filtered=products.filter(p=>(p.name+' '+p.category+' '+p.description).toLowerCase().includes(query.toLowerCase()));return <div className="container page"><PageHeading eyebrow="YOUR NEXT WORKFLOW STARTS HERE" title="Meet your next agent." description="A collection of tools for a little less routine and a lot more possibility."/><div className="catalog-toolbar"><span className="tab active">All agents <b>{products.length}</b></span><label className="search"><Search size={17}/><input aria-label="Search agents" placeholder="Find an agent…" value={query} onChange={e=>setQuery(e.target.value)}/></label></div><div className="product-grid">{filtered.map(p=><ProductCard key={p.id} product={p}/>)}</div>{!filtered.length&&<Empty title="No matches just yet." description="Try another name or clear your search."/>}<Notice>Product details are being prepared. Names, capabilities, and prices marked with an underscore are placeholders.</Notice><div className="inline-callout"><div><h3>Not sure where to start?</h3><p>Learn how purchase, download, and activation will work.</p></div><a className="text-button" href="/help">Get a little guidance <ArrowUpRight size={18}/></a></div></div>;}
  export function Detail({product}:{product:Product|undefined}){if(!product)return <NotFound/>;return <div className="container page"><a className="breadcrumb" href="/agents">← All agents</a><div className="detail-hero"><div><ProductIcon product={product} size={31}/><span className="category block">{product.category?.name || "Agent"} · {product.releaseStatus === 'development' ? 'In development' : product.releaseStatus === 'prelaunch' ? 'Preparing for launch' : 'Live'}</span><h1>{product.name}</h1><p className="lead">{product.tagline}</p><p>{product.description}</p><div className="detail-features">{(product.features||[]).map(f=><span key={f}><Check size={17}/>{f}</span>)}</div></div><aside className="purchase-panel"><span className="eyebrow">MAKE IT PART OF YOUR WORKDAY</span><h2>{product.price}</h2><p>billing_Term · usage_Allowance</p>{product.releaseStatus === 'development' ? <button className="button secondary full" disabled>In development — sales unavailable</button> : <a className="button primary full" href={'/checkout/'+product.slug}>{product.releaseStatus === 'live' ? 'Download now' : 'View availability'} <ArrowUpRight size={18}/></a>}<span className="small-note">Preview only. No real payments yet.</span><hr/><div><KeyRound size={17}/> Your own product activation key</div><div><Download size={17}/> Downloads in your account</div><div><ShieldCheck size={17}/> Account-based license management</div></aside></div><div className="two-column"><section className="panel"><h2>{product.workflowHeading || 'Built for your workflow.'}</h2><p>{product.workflowDescription || product.description}</p><div className="demo-placeholder" style={{ position: "relative", padding: 0, overflow: "hidden" }}>{product.workflowMediaUrl ? (product.workflowMediaUrl.match(/\.(mp4|webm)$/i) ? <video src={product.workflowMediaUrl} autoPlay muted loop playsInline style={{ width: "100%", height: "auto", display: "block" }} /> : <img src={product.workflowMediaUrl} alt="Walkthrough" style={{ width: "100%", height: "auto", display: "block" }} />) : <><Workflow size={40}/><h3>{product.name} walkthrough</h3><p>A product walkthrough will appear here.</p></>}</div>{product.workflowNote && <p className="small-note" style={{ marginTop: "1rem" }}>{product.workflowNote}</p>}</section><section className="panel"><h3>Before you get started</h3><dl><dt>System requirements</dt><dd>{product.requirements}</dd><dt>Current release</dt><dd>{product.version}</dd><dt>AI usage</dt><dd>usage_Policy — To be confirmed before launch.</dd><dt>Device allowance</dt><dd>One active device is the proposed starting policy.</dd></dl><a href="/help" className="text-button">Installation & activation <ArrowRight size={16}/></a></section></div></div>;}
