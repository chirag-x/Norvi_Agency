...
export function Workspace({ path, products, settings, onCatalogChange }: { path: string; products: Product[]; settings: Settings; onCatalogChange: () => void }) {
    ...
    const nav = admin ? staffNav.filter(([slug]) => allowed[user.role]?.includes('*') || allowed[user.role]?.includes(slug)) : customerNav;
    const sectionAllowed = !admin || allowed[user.role]?.includes('*') || allowed[user.role]?.includes(section);
    return <div className="workspace"><aside className="sidebar"><div className="workspace-label">{admin ? 'AGENCY WORKSPACE' : 'PERSONAL WORKSPACE'}</div><nav aria-label="Workspace navigation">{nav.map(([slug, label, Icon]) => <a key={slug} href={base + slug} className={section === slug ? 'selected' : ''}><Icon size={18} />{label}{section === slug && <span className="nav-indicator" />}</a>)}</nav><div className="sidebar-bottom"><div className="preview-box"><ShieldCheck size={18} /><b>{identity.preview ? 'Local preview' : 'NORVI account'}</b><p>{identity.preview ? 'Sample data. No live payments or emails.' : 'Verified account. Sales are not open yet.'}</p></div><a href="/" className="text-button">Back to website <ArrowUpRight size={15} /></a><button className="logout" onClick={async () => { try { await api('/auth/logout', { method: 'POST' }); location.href = admin ? '/admin/login' : '/login'; } catch (e: any) { setLogoutError(e.message); } }}><LogOut size={16} />{identity.preview ? 'Sign out of preview' : 'Sign out'}</button>{logoutError && <Notice kind="error">{logoutError}</Notice>}</div></aside><div className="workspace-main"><div className="workspace-topbar"><span>NORVI <span className="slash">/</span> {admin ? 'Agency workspace' : 'My account'}</span><span className="user-chip"><span className="avatar">{user.name.slice(0, 1).toUpperCase()}</span>{user.name}<Badge>{user.role.replace('_', ' ')}</Badge></span></div>{!sectionAllowed ? <Notice kind="error">Your role cannot open this section.</Notice> : admin ? !identity.preview ? <LiveAdmin section={section}/> : <Admin section={section} user={user} settings={settings} onCatalogChange={onCatalogChange} /> : <Customer section={section} products={products} user={user} />}</div></div>;
}
function Customer({ section, products, user }: { section: string; products: Product[]; user: User }) {
    const { data, error, reload } = useData('/account'); const [feedback, setFeedback] = useState(''), [keyModal, setKeyModal] = useState<{ key: string; name: string } | null>(null); if (error) return <Notice kind="error">{error}</Notice>; if (!data) return <Loading />; const licenses = data.licenses as any[], orders = data.orders as any[]; const owned = products.filter(p => licenses.some(l => l.productId === p.id)); const product = (id: string) => products.find(p => p.id === id); async function action(path: string) { try { const result = await api(path, { method: 'POST' }); if (result?.url) { window.location.href = result.url; return; } reload(); return result; } catch (e: any) { setFeedback(e.message); } }
...
function Admin({ section, user, settings, onCatalogChange }: { section: string; user: User; settings: Settings; onCatalogChange: () => void }) {
    const map: Record<string, string> = { '': 'products', '/products': 'products', '/customers': 'customers', '/licenses': 'licenses', '/orders': 'orders', '/subscriptions': 'billing', '/team': 'team', '/content': 'content', '/activity': 'audit', '/settings': 'settings' }; const endpoint = section === '' && user.role === 'support' ? 'customers' : map[section] || 'products'; const { data, error, reload } = useData('/admin/' + endpoint); const [query, setQuery] = useState(''), [filter, setFilter] = useState('all'), [editing, setEditing] = useState<Product | null | undefined>(undefined), [feedback, setFeedback] = useState(''), [licenseAction, setLicenseAction] = useState<{ id: string; action: string } | null>(null);
    const titles: Record<string, string> = { '': 'Your agency, at a glance.', '/products': 'Your agent collection.', '/customers': 'The people behind the accounts.', '/licenses': 'Access, under your control.', '/orders': 'Every order, in one place.', '/subscriptions': 'Keep track of recurring access.', '/team': 'Good work takes a team.', '/content': 'Make it sound like you.', '/activity': 'A clear record of every change.', '/settings': 'The details that make it yours.' };
    ...
    return <><PageHeading eyebrow={section === '' ? 'WELCOME TO YOUR WORKSPACE' : 'AGENCY MANAGEMENT'} title={titles[section] || 'Workspace'} description={section === '' ? 'Start with the essentials. Build your collection, get to know your customers, and keep things moving.' : undefined} action={section === '/products' ? <button className="button primary" onClick={() => setEditing(null)}><Plus size={17} />Add agent</button> : undefined} />{error ? <Notice kind="error">{error}</Notice> : !data ? <Loading /> : <>{feedback && <Notice>{feedback}</Notice>}
        ...
        {section === '/team' && <Team data={data} onAction={action} />}
